...
- Conventional-commit scopes are enforced via `commitlint.config.mjs` from `apps/*` and `packages/*` (path scopes like `apps/web` / `packages/db`, short aliases, plus meta: `docs`, `deps`, `ci`, `config`, `release`, `git`, `security`, `i18n`).
- Husky hooks: `commit-msg` runs commitlint; `pre-commit` runs lint-staged (Prettier on staged files; blocking `eslint --fix` for `apps`/`packages`); `pre-push` runs `bun run typecheck`. Interactive commits: `bun run commit` (Commitizen). Bypass with `--no-verify` or `HUSKY=0` when needed.
- Releases: `semantic-release` on `main` (`.github/workflows/release.yaml`, `release.config.js`) — updates `CHANGELOG.md` + root `package.json` version, GitHub release.
- Web i18n: `next-intl` (not next-translate) with Czech-only MVP catalog `apps/web/locales/cs.json`, typed via `AppConfig` + `createMessagesDeclaration`. Incremental UI migration; sidebar/header/metadata wired first.
