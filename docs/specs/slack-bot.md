...
flowchart LR
    ...
    SlackAPI -->|"POST signed body"| Route["apps/web<br/>app/api/slack/commands/route.ts"]
    Route -->|"200 within 3s<br/>Generuji fakturu"| SlackAPI
    Route -->|"after()"| Worker["Slack worker<br/>(Node runtime)"]
    Worker --> AISDK["AI SDK<br/>generateText({ tools, maxSteps })"]
    AISDK -->|"lookup_business"| Ares["@invoicey/ares"]
    AISDK -->|"compute_totals"| CoreT["invoice-core/totals"]
    AISDK -->|"assemble_and_validate"| CoreS["invoice-core/schema"]
    AISDK -->|"render_pdf"| CorePDF["invoice-core/pdf"]
    AISDK -->|"render_isdoc"| CoreX["invoice-core/isdoc"]
    Worker -->|"files.uploadV2 in thread"| SlackAPI
```

### Where it lives

- Receiver: route handler inside `apps/web` at **`apps/web/app/api/slack/commands/route.ts`** (Node runtime — `@react-pdf/renderer` needs Node, see [`specs/pdf-rendering.md`](./pdf-rendering.md)).
...
2. **Signature verification.** `slack-edge` verifies `X-Slack-Signature` + `X-Slack-Request-Timestamp` against `SLACK_SIGNING_SECRET`. Reject any timestamp older than 5 minutes.
3. **3-second ack.** Return `{ response_type: "ephemeral", text: "Generuji fakturu…" }`. Slack drops the connection if we don't ack in 3 s.
4. **Background work.** `after()` from `next/server` runs the AI worker after the response is sent.
5. **AI agent.** `generateText({ model, tools, system, prompt, maxSteps: 8 })` with the demo issuer + the user text in `prompt`. The model loops over tool calls until `assemble_and_validate` returns `ok: true`, then calls `render_pdf` and `render_isdoc`.
6. **Reply.** POST to `response_url` with a final message; in parallel, `chat.postMessage` + `files.uploadV2` for `invoice.pdf` and `invoice.isdoc.xml`, threaded under the original command (`thread_ts` derived from the channel where the slash command was triggered when available).
7. **Failure.** If the loop exits with `ok: false` or any tool throws, post a Czech-language ephemeral message via `response_url` listing missing/invalid fields. Raw issues go to the server log.
...
| --------------------------------------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Bad signature / stale timestamp                     | `slack-edge` verifier                 | HTTP 401, no Slack reply                                                                                                               |
...
| ---------------------------- | -------- | ------------------------ | ------------------------------------------------------------------- |
| `SLACK_BOT_TOKEN`            | yes      | —                        | `xoxb-…` bot OAuth, scopes: `commands`, `chat:write`, `files:write` |
...
Interactivity disabled for 13a — no buttons, no modals. 13b adds `interactive_components` for the "Issue" button.
...
| Signature verification | Reject stale timestamp / bad signature                                                                         | Vitest with `slack-edge`'s test helpers.                                                                                                                                                                                                                                                    |
| Smoke                  | `/invoice NFCtron 50000 retainer květen splatnost 14` against a real workspace via ngrok                       | Manual; capture screenshots in PR description when ready.                                                                                                                                                                                                                                   |
