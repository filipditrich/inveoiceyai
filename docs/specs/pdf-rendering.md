...
2. **Title band:** document label from `meta.docType` (Faktura / Proforma / Zálohová faktura / Dobropis), **`č.` / `No.`**, **`meta.number`** (e.g. `Faktura č. 20260119`). For a VAT-payer `invoice`, a micro-line **DAŇOVÝ DOKLAD** / **TAX DOCUMENT**. **Omit that line when `issuer.vatPayer === false`** — a non-payer invoice is not a tax document. Classic prints issue / due / performance date in the `dates` block on a sibling row under the parties (aligned with compact payment); looks without that block keep the dates in `title`.
