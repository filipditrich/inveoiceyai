...
| `list_invoices`                                                 | `{ limit?, unpaidOnly? }`                              | Summaries with `status` + `displayStatus` (needs `INVOICEY_DATABASE_URL`)                                                          |
...
- **Issuer lock:** `create_invoice` injects the workspace default issuer (`is_default`, else oldest). File-only MCP (no `INVOICEY_DATABASE_URL`) still uses `getDemoIssuer()`. Missing issuer with a database fails closed — demo is never persisted to Neon.
