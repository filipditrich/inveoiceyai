...
Make Invoicey fast and understandable for a Czech business to configure, issue and send invoices, understand their lifecycle, reconcile payments, and process incoming invoices. The work deepens existing product contracts; it does not replace the structured invoice model, accounting history, workspace isolation, or explicit human approval boundaries.
...
- Sending email, accepting incoming invoices, confirming bank matches, submitting payment batches, and cancellation remain explicit human actions.
- Active payment allocations block cancellation, with the invoice-row lock preserving the race invariant.
- Analytics requires measurement consent and never includes invoice text, customer/supplier identity, email address, invoice number, monetary amount, bank data, API keys, or document contents.
...
4. `feat(web): guide payment and incoming invoice work`
...
- Emit only state transitions: onboarding started/completed, invoice draft saved/recovered/issued, invoice email requested, payment match confirmed, and incoming invoice accepted/rejected.
...
## Lane 5 — guided payments and incoming work
...
- Give incoming queues a concise workload summary, due-date urgency, exception explanation, and a visible next action.
- Add an activity/progress surface to incoming detail using already persisted capture, extraction, review, approval, run, and payment states.
- Keep the existing approval engine and the separate proposed approval-workflow redesign out of this program.
...
- Incoming empty, exception, review, approval, and payment states each name the next action.
...
- Cover desktop and Pixel 7 for onboarding, new invoice recovery/lines, invoice detail delivery, payments, incoming queues, and app navigation.
...
| Actionable incoming workflow      | queue/detail state tests and browser evidence                                    |
...
- [x] `bun run test` — 7/7 tasks; web 37 files and 149 tests; invoice tools 11 files and 73 tests, including recipient suppression/transport guards, transactional payment-run eligibility/claim and relationship checks, malformed onboarding query recovery, claimed/terminal incoming guidance, credit-note payment exclusion, and incomplete-draft recovery.
...
- [ ] Implementation branch is merged to `main` without unrelated untracked files.
