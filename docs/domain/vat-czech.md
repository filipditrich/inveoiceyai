...
How Invoicey models Czech VAT. This is one of the highest-risk domain areas: incorrect VAT on an invoice has tax-authority consequences. Anything ambiguous below should be resolved with reference to *zákon č. 235/2004 Sb. o dani z přidané hodnoty* (the VAT Act) and a CPA — not by reading more docs.
...
| **12 %** | snížená sazba (reduced) | Selected items per Annexes 2 and 3 of the VAT Act: tap water, heat/cooling, accommodation, restaurant catering (food, *not* alcohol), passenger transport, residential construction, repairs of housing, certain food categories |
...
- Charges the *destination country's* VAT rate
...
`InvoiceItemSchema.vatRate` is currently a flat number. For OSS we need to track *which country's* rate it is, which matters for ISDOC. Either (a) extend each item with `vatCountry`, or (b) attach `vatCountry` to the invoice-level `vat` block. Decision lands during Plan 2.
...
We default to `duzp = issueDate` for both `proforma` and `advance` because the schema requires DUZP non-optional. The PDF can choose to *not render* DUZP for these doc types. ISDOC sets the doc type accordingly so importers handle it correctly.
...
| --- | --- |
| `true` | Invoice is a *daňový doklad*. VAT lines, breakdown, DIČ printed, ISDOC includes `<TaxTotal>` |
| `false` | Invoice is *not a tax document*. No DIČ rendered, every line `vatRate = 0`, PDF says "Nejsem plátce DPH" |
...
- Issuer: Jan Novák (OSVČ, *not* vatPayer)
...
> Note: cross-border-currency invoices need multi-currency support, which is post-MVP. For MVP we either (a) decline EU-cross-border B2B until multi-currency lands or (b) invoice in CZK using a CNB rate the issuer pre-applies. The schema accepts the data; the *UX* postpones the case.
...
Identified-person status sits between *plátce* and *neplátce*. They have a DIČ, don't charge domestic VAT, but participate in VIES/EU reverse charge. Modeled implicitly today (issuer marked `vatPayer = false` but with a `dic`). Decide if we add an explicit flag in MVP polish.
