...
async function loadIssuer(workspaceId: string, issuerId: string) {
	const rows = await db
		...
		.limit(1);
	const snap = rows[0] ? IssuerSnapshotSchema.safeParse(rows[0].snapshot) : null;
	if (!snap?.success) {
		return null;
	}
	return snap.data;
...
async function uploadIsdocXml(number: string, xml: string): Promise<string | null> {
...
export async function commitInvoiceImport(input: {
	...
	const providerParse = InvoiceOriginProviderSchema.safeParse(input.originProvider);
		...
		for (const item of chunk) {
			try {
				...
				if (item.isdocXml && item.completeness === "full" && item.invoice) {
					isdocUrl = await uploadIsdocXml(item.invoice.meta.number, item.isdocXml);
				}
