...
/**
 * Persist credit movements from a normalized batch and run deterministic
 * matching (optional exact auto-confirm). Shared by Fio and Moneta sync.
 */
export async function importBankTransactionBatch(input: {
  ...
  const credits = input.batch.transactions.filter(
