...
export function InvoiceCollectionWaiting(details: CollectionDetails) {
  const t = useTranslations("Invoices.collect");
    ...
    async function heartbeat(): Promise<void> {
      try {
        const response = await fetch(
          `/api/invoices/${details.invoiceId}/collection-status`,
          { method: "POST", cache: "no-store" },
        );
        if (!response.ok)
          throw new Error(`collection_status_${response.status}`);
  ...
  }, [details.invoiceId, status.settled]);
        ...
        <div className="space-y-5">
          <div className="space-y-1.5">
            <p className="text-sm font-medium text-muted-foreground">
              {t("invoice", { number: details.number })}
            </p>
            <h2 className="font-heading text-3xl font-semibold tracking-tight sm:text-4xl">
              {formatMoney(requested, details.currency, locale)}
            </h2>
            <p className="text-sm text-muted-foreground">
              {t("forClient", { client: details.clientName })}
            </p>
          </div>
