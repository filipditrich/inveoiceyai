...
function Sidebar({
  ...
  const tA11y = useTranslations("App.a11y");
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{tA11y("sidebarTitle")}</SheetTitle>
            <SheetDescription>{tA11y("sidebarDescription")}</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className="sr-only">{tA11y("toggleSidebar")}</span>
    </Button>
  );
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      aria-label={tA11y("toggleSidebar")}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={tA11y("toggleSidebar")}
      className={cn(
