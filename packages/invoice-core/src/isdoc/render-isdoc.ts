...
function appendAccountingSupplierParty(root: Xm, invoice: Invoice) {
	...
	party.ele(NSDOC, "PartyName").ele(NSDOC, "Name").txt(invoice.issuer.name).up().up();
	...
	if (invoice.issuer.registryNote?.trim()) {
		...
		pref.ele(NSDOC, "Preformatted").txt(invoice.issuer.registryNote.trim()).up();
		pref.up();
	}
...
function appendClassifiedTaxCategory(
	...
	if (invoice.issuer.vatPayer || isReverseCharge(invoice) || isOss(invoice)) {
		bucket.ele(NSDOC, "VATApplicable").txt(isoBoolean(invoice.issuer.vatPayer)).up();
	}
...
function appendInvoiceLines(root: Xm, invoice: Invoice) {
	...
	for (const line of sorted) {
		...
		li.ele(NSDOC, "UnitPriceTaxInclusive").txt(money(unitInclusive(line))).up();
...
function appendTaxTotal(root: Xm, invoice: Invoice) {
	...
	for (const row of invoice.totals.vatBreakdown) {
		...
		st.ele(NSDOC, "TaxInclusiveAmount").txt(money(row.base + row.vat)).up();
		...
		st.ele(NSDOC, "DifferenceTaxInclusiveAmount").txt(money(row.base + row.vat)).up();

		const tc = st.ele(NSDOC, "TaxCategory");
		tc.ele(NSDOC, "Percent").txt(String(isReverseCharge(invoice) ? 0 : row.rate)).up();
...
function appendPaymentMeans(root: Xm, invoice: Invoice) {
	...
	pays.ele(NSDOC, "PaymentMeansCode").txt(String(paymentMeansInteger(invoice))).up();
	...
	if (
		...
		const accParsed = parseCzAccountNumber(invoice.payment.bankAccount.accountNumber);
		...
		details.ele(NSDOC, "BIC").txt(invoice.payment.bankAccount.bic?.trim() ?? "").up();
