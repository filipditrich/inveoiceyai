...
import type { Invoice, IssuerSnapshot } from "../schema";
...
function child(node: unknown, localName: string): unknown {
	...
	for (const [k, v] of Object.entries(rec)) {
		if (k === localName || k.endsWith(`:${localName}`) || k.endsWith(`}${localName}`)) {
			return v;
		}
	}
	return undefined;
...
function partyAddress(party: unknown): Invoice["client"]["address"] {
	...
	const country =
		textOf(child(child(addr, "Country"), "IdentificationCode")).toUpperCase() || "CZ";
	return {
 ...
 */
export function parseIsdoc(xml: string, options: ParseIsdocOptions): ParseIsdocResult {
	...
	const items: Invoice["items"] = lineNodes.map((line, index) => {
		...
		const lineTotal = numOf(child(line, "LineExtensionAmountTaxInclusive"), lineSubtotal + lineVat);
		const unitPrice = numOf(child(line, "UnitPrice"), qty !== 0 ? lineSubtotal / qty : 0);
		const description =
			textOf(child(child(line, "Item"), "Description")) || `Položka ${index + 1}`;
		return {
	...
	if (items.length === 0) {
		const payable = numOf(child(child(root, "LegalMonetaryTotal"), "PayableAmount"));
		const exclusive = numOf(
			child(child(root, "LegalMonetaryTotal"), "TaxExclusiveAmount"),
			payable,
		);
		const tax = numOf(child(child(root, "TaxTotal"), "TaxAmount"), payable - exclusive);
		items.push({
			position: 1,
	...
	for (const st of children(child(root, "TaxTotal"), "TaxSubTotal")) {
		const rate = Math.round(numOf(child(child(st, "TaxCategory"), "Percent"), 0));
	...
	if (method === "transfer") {
		payment.bankAccount =
			accountNumber && /^CZ\d{22}$/.test(iban)
				? {
						accountNumber:
							accountNumber || options.issuer.bank.accountNumber,
	...
	const corrected = textOf(
		child(children(child(root, "OriginalDocumentReferences"), "OriginalDocumentReference")[0], "ID"),
	);
	...
	const draft: Invoice = {
		meta: {
			...
			language: "cs",
			currency: "CZK",
			correctedInvoiceNumber: docType === "credit_note" ? corrected || number : undefined,
		},
