...
export async function fetchAresEkonomickySubjekt(
	...
	if (response.status === 404) {
		...
		if (nf.ok && nf.val.kod === "NENALEZENO") {
			return {
				...
				kind: "not_found",
				message:
					nf.val.popis?.split("|")[0]?.trim() ?? "Ekonomický subjekt nenalezen.",
				httpStatus: 404,
			};
		}
	...
	if (!parsed.data.sidlo || !name) {
		return {
			...
			kind: "invalid_response",
			message:
				"ARES vrátilo subjekt bez obchodního jména nebo adresy sídla.",
			httpStatus: response.status,
		};
	}
...
async function tryParseNotFoundBody(
	response: Response,
): Promise<{ ok: true; val: z.infer<typeof AresNotFoundBodySchema> } | { ok: false }> {
	try {
